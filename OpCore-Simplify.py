...
class OCPE:
    def __init__(self):
        self.u = utils.Utils("OpCore Simplify-ZH-CN-Translator-v3.0.0")
    ...
    def select_hardware_report(self):
        ...
        while True:
            self.u.head("选择硬件报告（.JSON）")
            print("")
            if os.name == "nt":
                print("\033[1;93mNote:\033[0m")
                print("- 在生成硬件报告之前，请确保您正在使用最新版本的硬件嗅探器。")
                print("- 硬件嗅探器不会收集与可调整 BAR 选项（默认禁用）和 Windows PE 中的显示器连接相关的信息。")
                print("")
                print("E. 导出硬件报告（推荐）")
                print("")
            print("Q. 退出程序")
            ...
            if user_input.lower() == "e":
                ...
                self.u.head("导出硬件报告（推荐）")
                print("")
                print("导出硬件报告到 {}...".format(report_dir))
                ...
                if output[-1] != 0:
                    ...
                    print("")
                    print("导出硬件报告失败。 {}".format(error_message))
                    print("请重试或手动使用硬件嗅探器。")
                    print("")
            ...
            if not is_valid or errors:
                print("")
                print("\033[32m建议:\033[0m 请重新导出硬件报告并尝试再次。")
                print("")
    ...
    def show_oclp_warning(self):
        while True:
            ...
            print("")
            print("1. OpenCore Legacy Patcher 是唯一支持在较新的 macOS 版本中启用已弃用 GPU 和 Broadcom WiFi")
            print("   支持，以及为 macOS Tahoe 26 带来 AppleHDA 的唯一解决方案。")
            print("")
            print("2. OpenCore Legacy Patcher 禁用了一些 macOS 安全功能，如 SIP 和 AMFI。这可能会导致一些问题，")
            print("   例如需要下载完整的更新安装程序、应用程序崩溃和系统不稳定性。")
            print("")
            print("3. OpenCore Legacy Patcher  不支持 Hackintosh 社区。")
            print("")
            print("3. OpenCore Legacy Patcher  不支持 Tahoe Hackintosh 社区。")
            print("")
            print("\033[1;91m重要:\033[0m")
            print("请继续前仔细考虑这些风险，避免可能的问题。")
            print("")
            print("\033[1;96m支持 macOS Tahoe 26 支持:\033[0m")
            print("要为 macOS Tahoe 26 应用补丁，您必须下载 OpenCore-Patcher 3.0.0 或更高版本。")
            print("我的仓库: \033[4mlzhoang2801/OpenCore-Legacy-Patcher\033[0m")
            print("注意!:翻译工程师发现上面的仓库不可用!")
            print("旧版或官方版 Dortania 发布版本 不支持 Tahoe 26。")
            print("")
            option = self.u.request_input("是否继续使用 OpenCore Legacy Patcher？ (是/否): ").strip().lower()
            if option == "是":
                return True
            elif option == "否":
                return False
    ...
    def select_macos_version(self, hardware_report, native_macos_version, ocl_patched_macos_version):
        ...
        for device_type in ("图形计算核心", "网络", "蓝牙", "SD 控制器"):
            if device_type in hardware_report:
                for device_name, device_props in hardware_report[device_type].items():
                    if device_props.get("Compatibility", (None, None)) != (None, None):
                        if device_type == "图形计算核心" and device_props.get("Device Type") == "集成的图形计算核心":
        ...
        while True:
            ...
            if native_macos_version[1][:2] != suggested_macos_version[:2]:
                print("")
                print("\033[1;36m建议 macOS 版本:\033[0m")
            ...
            print("Q. 退出")
            print("")
            option = self.u.request_input("请输入要使用的 macOS 版本（默认： {}）：".format(os_data.get_macos_name_by_darwin(suggested_macos_version))) or suggested_macos_version
            if option.lower() == "q":
                self.u.exit_program()
    ...
    def build_opencore_efi(self, hardware_report, disabled_devices, smbios_model, macos_version, needs_oclp):
        ...
        if not config_data:
            raise Exception("文件 {} 不存在。".format(config_file))
    ...
    def check_bios_requirements(self, org_hardware_report, hardware_report):
        ...
        if org_firmware_type == "Legacy" and firmware_type == "UEFI":
            requirements.append("启用 UEFI 模式（禁用 Legacy/CSM）")

        secure_boot = hardware_report.get("BIOS", {}).get("Secure Boot", "Unknown")
        if secure_boot != "Disabled":
            requirements.append("禁用 UEFI 模式（禁用 Secure Boot）")
        ...
        if hardware_report.get("Motherboard", {}).get("Platform") == "Desktop" and hardware_report.get("Motherboard", {}).get("Chipset") in chipset_data.IntelChipsets[112:]:
            ...
            if not resizable_bar_enabled:
                requirements.append("启用 Above 4G 解码")
    ...
    def before_using_efi(self, org_hardware_report, hardware_report):
        while True:
            ...
            print("    - 将 UTBMap.kext 添加到 {} 文件夹。".format("EFI\\OC\\Kexts" if os.name == "nt" else "EFI/OC/Kexts"))
            print("    - 从 {} 文件夹删除 UTBDefault.kext。".format("EFI\\OC\\Kexts" if os.name == "nt" else "EFI/OC/Kexts"))
            print("    - 编辑 config.plist：")
            print("        - 使用 ProperTree 打开 config.plist。")
            print("        - 按下 Command/Ctrl + R 运行 OC。")
            print("        - 如果控制器上端口数量超过 15 个，启用 XhciPortLimit patch。")
            print("        - 保存文件。")
            print("")
    ...
    def main(self):
        ...
        while True:
            self.u.head()
            print("")
            print("  硬件报告：    {}".format(hardware_report_path or 'Not selected'))
            if hardware_report_path:
                print("")
                print("  macOS 版本：    {}".format(os_data.get_macos_name_by_darwin(macos_version) if macos_version else 'Not selected') + (' (' + macos_version + ')' if macos_version else '') + ('. \033[1;93mRequires OpenCore Legacy Patcher\033[0m' if needs_oclp else ''))
            ...
            if option == "2":
                ...
                self.u.head("结果")
                print("")
                print("您的 OpenCore EFI 已构建在 {} 文件夹中：".format(customized_hardware.get("Motherboard").get("Name")))
