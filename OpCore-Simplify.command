...
download_py () {
    ...
    if [ "$?" != "0" ]; then
        echo
        echo " - 无法下载 Python 安装程序!"
        echo
        exit $?
    fi
    echo
    echo "安装 Python $vers 包..."
    echo
    sudo installer -pkg "$tempdir/python.pkg" -target /
    if [ "$?" != "0" ]; then
        echo
        echo " - 无法安装 Python $vers!"
        echo
        exit $?
    fi
...
print_error() {
    ...
    echo "###                      ###"
    echo
    echo "Python 未安装或未在 PATH 变量中找到。"
    echo
    if [ "$kernel" == "Darwin" ]; then
        echo "请访问 https://www.python.org/downloads/macos/ 下载最新版本，然后重试。"
    else
        echo "请通过您的包管理器安装 python 然后再试。"
    fi
    echo
    exit 1
}
...
prompt_and_download() {
    ...
    echo "无法找到 $printed_py!"
    echo
    echo "此脚本需要 $printed_py 运行。"
    echo
    while true; do
        read -p "是否立即安装最新 $target_py？ (y/n):  " yn
        case $yn in
