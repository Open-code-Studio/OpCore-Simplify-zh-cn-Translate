...
class DSDT:
    def __init__(self, **kwargs):
        ...
        if not self.iasl:
            ...
            if url:
                exception += "\n\n请手动从 {} 下载:\n - {}\n\n并将其放置在:\n - {}\n".format(
                    "\"iasl-win-YYYYMMDD.zip\" 并提取出 iasl.exe" if os.name=="nt" else "iasl",
                    url,
        ...
        try:
            ...
            if not valid_files:
                ...
                raise FileNotFoundError(
                    ...
                    "在 {} 中未找到有效 .aml/.dat 文件".format(table_path)
                )
    ...
    def dump_tables(self, output, disassemble=False):
        ...
        if os.name == "nt":
            ...
            if os.path.exists(target):
                ...
                print("未找到 acpidump.exe")
                return
        elif sys.platform.startswith("linux"):
            table_dir = "/sys/firmware/acpi/tables"
            if not os.path.isdir(table_dir):
                print("未找到 {}！".format(table_dir))
                return
            print("正在复制表格到 {}...".format(res))
            copied_files = []
    ...
    def get_unique_pad(self, current_hex, index, direction=None, instance=0, table=None):
        if not table: table = self.get_dsdt_or_only()
        if not table: raise Exception("未找到有效表格！")
        ...
        if last_index == -1:
            raise Exception("未找到以索引 {} 开始的 {}！".format(index,current_hex))
        first_line = line
        ...
        while True:
            ...
            if last_index == -1:
                raise Exception("未找到 {} ！".format(current_hex))
            # Append the new info
            line += new_line
        if not current_hex in line:
            raise Exception("{} 未在表格中找到！".format(current_hex))
        padl = padr = ""
        parts = line.split(current_hex)
        if instance >= len(parts)-1:
            raise Exception("实例索引超出范围！")
        ...
        while True:
            ...
            if direction == True or (direction is None and len(padr)<=len(padl)):
                ...
                if not len(liner):
                    ...
                    if last_index == -1: raise Exception("在找到唯一十六进制值之前已到达文件末尾！")
                padr  = padr+liner[0:2]
                liner = liner[2:]
                continue
            if direction == False or (direction is None and len(padl)<=len(padr)):
                ...
                if not len(linel):
                    ...
                    if _index == -1: raise Exception("在找到唯一十六进制值之前到达了文件末尾！")
                padl  = linel[-2:]+padl
                linel = linel[:-2]
                continue
            break
