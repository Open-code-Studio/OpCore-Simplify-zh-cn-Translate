...
class ACPIGuru:
    ...
    def read_acpi_tables(self, path):
        ...
        if os.path.isdir(path):
            ...
            if len(dsdt_list) > 1:
                print("多个文件包含 DSDT 签名：")
                for d in self.sorted_nicely(dsdt_list):
                    print(" - {}".format(d))
                print("\n仅允许加载一个 DSDT 文件，请删除其中一个并重试。")
                print("")
                #self.u.grab("Press [enter] to return...")
                self.utils.request_input()
                # Restore any prior tables
            ...
            if not self.acpi._table_signature(path) == "DSDT":
                ...
                print("\n{} 无法解编！".format(os.path.basename(path)))
                print("")
                #self.u.grab("Press [enter] to return...")
                self.utils.request_input()
                # Restore any prior tables
        ...
        if trouble_dsdt:
            ...
            print("正在检查可用的预补丁...")
            print("正在加载 {}...".format(trouble_dsdt))
    ...
    def get_sta_var(self,var="STAS",device=None,dev_hid="ACPI000E",dev_name="AWAC",log_locate=False,table=None):
        ...
        if device:
            ...
            if log_locate: print("正在定位 {} ({}) 设备中...".format(dev_hid,dev_name))
        ...
        for table_name in self.sorted_nicely(list(self.acpi.acpi_tables)):
            ...
            if not skip_ec:
                ...
                if len(ec_list):
                    ...
                    if log: print(" - 找到 {}！".format(lpc_name)) 
                    return lpc_name
